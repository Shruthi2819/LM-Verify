...
function AIVerificationAssistant({
  analysis,
  loading,
  warnings = [],
  ...
  const [chatHistory, setChatHistory] = useState([
    {
      sender: "AI",
      text: `Hello Officer, I have loaded the calibration parameters for this ${instrumentType}. Ask me anything about standard tolerances or checks.`
    }
  ]);
  ...
  const handleSend = (e) => {
    ...
    setChatHistory((prev) => [...prev, { sender: "LMO", text: userMsg }]);
    setQuery("");
    ...
    setTimeout(() => {
      ...
      if (text.includes("seal") || text.includes("flag")) {
        ...
        reply = "For Class III weighing scales, the maximum permissible error (MPE) is ±0.5 scale intervals. Deviations beyond this require recalibration before approval.";
  ...
  const getRiskColor = (risk) => {
    switch (risk) {
      case "LOW": return "bg-green-55 text-green-800 border-green-200";
  ...
  if (loading) {
    return (
      <Card className="border-blue-100 bg-blue-50/5/5">
  ...
  if (!analysis) {
    return (
      <Card className="text-center py-8">
        ...
        <p className="text-xs text-slate-400">AI verification analysis not generated yet.</p>
        <p className="text-[10px] text-slate-400 mt-1">Open the inspection workspace or trigger explicit checks.</p>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="border-blue-200 bg-blue-50/5">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
          ...
          <div className="text-right">
            ...
            <span className="text-xs font-bold text-slate-800 select-none" title="Confidence represents statistical matching rating of the input dataset.">
              {analysis.confidence}%
            </span>
          </div>
        </div>
      ...
      <Card>
        <Card.Header className="border-b border-slate-100 pb-2 mb-3">
          <h4 className="text-[11px] font-bold text-slate-800 uppercase tracking-wider">Device Checks Guidance</h4>
        </Card.Header>
          ...
          {analysis.checklistRecommendations.map((check, idx) => (
            <div key={idx} className="flex items-start gap-2 text-slate-600 text-[11px] leading-relaxed">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">Reason: Ensure calibration consistency across standard regulatory classes.</p>
              </div>
            </div>
          ))}
        </Card.Body>
      ...
      <Card>
        <Card.Header className="border-b border-slate-100 pb-2 mb-3 flex items-center gap-1.5">
          ...
          <h4 className="text-[11px] font-bold text-slate-800 uppercase tracking-wider">Verification Guide Q&A</h4>
        </Card.Header>
        <Card.Body className="space-y-3">
          <div className="h-40 overflow-y-auto border border-slate-100 rounded bg-slate-50 p-2.5 space-y-2 text-[10px]">
            {chatHistory.map((msg, idx) => (
              <div key={idx} className={msg.sender === "LMO" ? "text-right" : "text-left"}>
                <span className={["inline-block p-1.5 rounded max-w-[85%] leading-relaxed", msg.sender === "LMO" ? "bg-blue-600 text-white text-right" : "bg-white border border-slate-200 text-slate-700"].join(" ")}>
                  {msg.text}
                </span>
              </div>
            ))}
          ...
          <form onSubmit={handleSend} className="flex gap-1.5">
            <input
              type="text"
              placeholder="Ask about seals or tolerances..."
              value={query}
